...
int get_reg_cmd(FILE  *fp)
{
	...
	do {
		...
		count = sscanf(line, "%s %d %d %d %d", cmd,
			...
			(unsigned int *) &num[2],
			(unsigned int *) &num[3]);
		hcount = sscanf(line, "%s %x %x %x %x %x %x %x %x", cmd,
			(unsigned int *) &hex[0],
			(unsigned int *) &hex[1],
			(unsigned int *) &hex[2],
			(unsigned int *) &hex[3],
		...
		case 'r':
			if (hcount >= 2) {
				int i;
				sw_store.reg_info.reg = hex[0];
				sw_store.reg_info.cnt = 1;
				if (hcount >= 3)
					sw_store.reg_info.cnt = hex[1];
				sw_store.reg_info.param = 0;
				if (line[1] == 'd') {
					sw_store.reg_info.dword = 1;
				} else if (line[1] == 'w') {
					sw_store.reg_info.word = 1;
				} else if (line[1] == 'b') {
					sw_store.reg_info.byte = 1;
				}
		...
		case 'w':
			if (hcount >= 3) {
				...
				if (line[1] == 'd') {
					...
					size = sw_store.reg_size;
				sw_store.reg_info.reg = hex[0];
				for (len = 0; len < hcount - 2; len++) {
					if (size == 4)
						sw_store.reg_info.val.dword[len] =
							hex[len + 1];
					else if (size == 2)
						sw_store.reg_info.val.word[len] =
							(u16) hex[len + 1];
					else
						sw_store.reg_info.val.byte[len] =
							(u8) hex[len + 1];
				}
				sw_store.reg_info.cnt = len;
				sw_store.reg_info.write = 1;
				w_sw_regs(fd, &sw_store);
			}
			break;
		case 'h':
	...
	do {
		...
		default:
			...
			if (len >= 2) {
				len--;
				line[len] = '\0';
				sw_store.sysfs_info.str_len = len;
				strncpy(sw_store.sysfs_info.str, line, len);
				w_sw_sysfs(fd, &sw_store);
				wait_for_resp(&sw_store.sysfs_cont);
			}
			break;
		}
	} while (cont);
...
#endif
	if (DEV_MOD_BASE == msg->hdr.mod) {
		rc = proc_sw_msg(net_info, &sw_store);
		if (1 == rc) {
#if 0
			proc_sw_resp(net_info, &info);
#endif
		}
	}
}  /* proc_msg */
...
int main(int argc, char *argv[])
{
	...
	init_sw_info(&net_info, &sw_store);
	if ( !param[0].fTaskStop ) {
		...
		do {
			switch (rc) {
			case 1:
				rc = get_sysfs_cmd(stdin);
				break;
			case 2:
				rc = get_reg_cmd(stdin);
				break;
			default:
				rc = 1;
				break;
