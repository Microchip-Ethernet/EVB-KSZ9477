...
static void setup_dsa_switch(struct macb *bp)
{
	ksz_switch_init(&ksz_switch_plat_data, NO_IRQ,
		&bp->dev->dev, &bp->phy_dev->bus->dev, bp->port.sw);
}
#endif
...
static void macb_tx_unmap(struct macb *bp, struct macb_tx_skb *tx_skb)
{
	...
	if (tx_skb->skb) {
#ifdef CONFIG_1588_PTP
		if (skb_shinfo(tx_skb->skb)->tx_flags & SKBTX_IN_PROGRESS)
			skb_shinfo(tx_skb->skb)->tx_flags &=
				!SKBTX_HW_TSTAMP;
		else
#endif
...
static void macb_tx_error_task(struct work_struct *work)
{
	...
	wake_dev_queues(bp->port.sw, bp->dev);
#endif
...
static void macb_tx_interrupt(struct macb *bp)
{
...
#ifdef HAVE_KSZ_SWITCH
	queue_stopped = netif_queue_stopped(bp->dev);
#endif
	bp->tx_tail = tail;
	...
	if (queue_stopped && !netif_queue_stopped(bp->dev))
		wake_dev_queues(bp->port.sw, bp->dev);
#endif
}
...
#endif
	memset(bp->counters, 0, sizeof(bp->counters));
...
#ifdef CONFIG_KSZ_IBA_ONLY
	if (bp->phy_dev == &hbp->dummy_phy || sw_host_port)
...
static int macb_close(struct net_device *dev)
{
	...
	do {
		...
		if (sw_is_switch(sw)) {
			...
			if (bp->phy_dev != bp->saved_phy) {
				if (bp->phy_dev->bus)
					phy_detach(bp->phy_dev);
				bp->phy_dev = bp->saved_phy;
				if (bp->phy_dev && bp->phy_dev->bus)
					phy_stop(bp->phy_dev);
			}
			ksz_remove(sw->dev);
...
#ifdef HAVE_KSZ_SWITCH
	if (sw_is_switch(bp->port.sw) && phydev->bus) {
		int rc = mii_ethtool_gset(&bp->mii_if, cmd);
...
#ifdef HAVE_KSZ_SWITCH
	if (sw_is_switch(bp->port.sw) && phydev->bus)
		return mii_ethtool_sset(&bp->mii_if, cmd);
#endif
...
} ethtool_macb_stats_keys[sizeof(struct macb_stats) / 4] = {
	{ "rx_pause_packets" },
	{ "tx_packets" },
...
static struct {
	char string[ETH_GSTRING_LEN];
} ethtool_gem_stats_keys[sizeof(struct gem_stats) / 4] = {
	{ "tx_lo_octets" },
	{ "tx_hi_octets" },
	{ "tx_packets" },
	{ "tx_bcast_packets" },
	...
	{ "tx_pause_packets" },
	{ "tx_64_octet_packets" },
	{ "tx_65_to_127_octet_packets" },
	{ "tx_128_to_255_octet_packets" },
	{ "tx_256_to_511_octet_packets" },
	{ "tx_512_to_1023_octet_packets" },
	{ "tx_1024_to_1518_octet_packets" },
	{ "tx_1519_octet_packets" },
	{ "tx_underuns" },
	{ "tx_single_collisions" },
	{ "tx_mult_collisions" },
	{ "tx_excessive_collisions" },
	...
	{ "rx_1024_to_1518_octet_packets" },
	{ "rx_1519_octet_packets" },
	{ "rx_fragments" },
	{ "rx_oversize_packets" },
	{ "rx_jabbers" },
	{ "rx_crc_errors" },
...
static void macb_get_strings(struct net_device *dev, u32 stringset, u8 *buf)
{
	struct macb *bp = netdev_priv(dev);
	int n;
	void *ptr;
	...
	if (macb_is_gem(bp)) {
		n = sizeof(struct gem_stats) / 4;
		ptr = &ethtool_gem_stats_keys;
	} else {
		n = sizeof(struct macb_stats) / 4;
		ptr = &ethtool_macb_stats_keys;
	}
	if (ETH_SS_STATS == stringset)
		memcpy(buf, ptr, ETH_GSTRING_LEN * n);
}
	...
	int port_count, int mib_port_count, char *dev_name)
{
	...
	if (!IS_ERR(phydev)) {
		...
		if ((bp->phy_dev->drv->features & PHY_GBIT_FEATURES) ==
		     PHY_GBIT_FEATURES)
...
static int macb_sw_init(struct macb *bp)
{
	...
	if (bp->phy_dev->bus)
		bp->phy_dev->adjust_link = macb_handle_link_change;
	...
	for (i = 1; i < dev_count; i++) {
		...
		bp->hw_priv = hw_priv;
		bp->phy_dev = &bp->dummy_phy;
		bp->phy_dev->duplex = 1;
		bp->phy_dev->speed = SPEED_1000;
...
static int create_dummy_phy(struct macb *bp)
{
	bp->phy_dev = &bp->dummy_phy;
	bp->phy_dev->link = 1;
	bp->phy_dev->duplex = 1;
	bp->phy_dev->speed = SPEED_1000;
	bp->phy_dev->pause = 1;
	bp->phy_dev->interface = bp->phy_interface;
	bp->phy_dev->attached_dev = bp->dev;
	bp->phy_dev->adjust_link = macb_handle_link_change;
	bp->hw_priv = bp;
	return 0;
}  /* create_dummy_phy */
...
static int create_sw_dev(struct net_device *dev, struct macb *bp)
{
	...
	/* Direct connection. */
	if (bp->phy_dev == &bp->dummy_phy)
		bp->phy_dev->adjust_link(bp->phy_dev->attached_dev);
	return 0;
}  /* create_sw_dev */
#endif
