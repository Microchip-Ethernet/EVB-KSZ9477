...
#define KS8842_RESERVED_1_P		0x0550
#define KS8842_RESERVED_2_P		0x0554
#define KS8842_RESERVED_3_P		0x0556
#define KS8842_RESERVED_4_P		0x0560
...
static void port_set_link_speed(struct ksz_port *port)
{
	...
	for (i = 0, p = port->first_port; i < port->port_cnt; i++, p++) {
		...
		cfg = data;
		if ((status & 0x1f) && !(status & PORT_STATUS_LINK_GOOD))
			cfg = 0;
...
static int hw_init(struct ksz_hw *hw)
{
	...
	/*
	 * Clear additional MAC addresses which are filled with random values
	 * on power up.
	 */
	data = 0;
	for (revision = 0; revision < ADDITIONAL_ENTRIES; revision++) {
		writel(0, hw->io + data + KS_ADD_ADDR_0_LO);
		writel(0, hw->io + data + KS_ADD_ADDR_0_HI);
		data += ADD_ADDR_INCR;
	}
	return rc;
}
...
static void hw_read_addr(struct ksz_hw *hw)
{
	...
	if (!hw->mac_override) {
		...
		if (empty_addr(hw->override_addr) ||
		    !is_valid_ether_addr(hw->override_addr)) {
...
static struct hw_regs {
	...
	{ KS8842_RESERVED_1_P,	KS8842_RESERVED_4_P },
	{ 0, 0 }
};
...
#define KSZ884X_REGS_SIZE	0x580
...
static ssize_t ksz884x_registers_read(struct file *filp, struct kobject *kobj,
	struct bin_attribute *bin_attr, char *buf, loff_t off, size_t count)
{
	...
	if (unlikely(off > KSZ884X_REGS_SIZE))
		return 0;

	if ((off + count) > KSZ884X_REGS_SIZE)
		count = KSZ884X_REGS_SIZE - off;
	...
	for (i = 0; i < count; i += 4, reg += 4, addr++) {
		*addr = 0;
		if (check_hw_reg_range(reg)) {
			if (count >= 4)
				*addr = readl(hw->io + reg);
			else if (count >= 2)
				*addr_16 = readw(hw->io + reg);
			else
				*addr_8 = readb(hw->io + reg);
		}
	}
	return i;
}

static ssize_t ksz884x_registers_write(struct file *filp, struct kobject *kobj,
	struct bin_attribute *bin_attr, char *buf, loff_t off, size_t count)
{
	size_t i;
	u32 *addr;
	...
	for (i = 0; i < count; i += 4, reg += 4, addr++) {
		if (check_hw_reg_range(reg)) {
			if (count >= 4)
				writel(*addr, hw->io + reg);
			else if (count >= 2)
				writew(*addr_16, hw->io + reg);
			else
				writeb(*addr_8, hw->io + reg);
		}
	}
	return i;
}

static struct bin_attribute ksz884x_registers_attr = {
	.attr = {
		.name	= "registers",
		.mode	= S_IRUSR | S_IWUSR,
	},
	.size	= KSZ884X_REGS_SIZE,
	.read	= ksz884x_registers_read,
	.write	= ksz884x_registers_write,
};
...
static int pcidev_init(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	dev = info->netdev[0];
	result = sysfs_create_bin_file(&dev->dev.kobj, &ksz884x_registers_attr);
...
static void pcidev_exit(struct pci_dev *pdev)
{
	...
	if (info->netdev[0]) {
		struct net_device *dev = info->netdev[0];

		sysfs_remove_bin_file(&dev->dev.kobj, &ksz884x_registers_attr);
	}
