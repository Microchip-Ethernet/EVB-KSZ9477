...
static int make_hmac_md5(void *in_data, int len, u8 *out_data)
{
	...
	hmac_md5 = crypto_alloc_hash("hmac(md5)", 0, CRYPTO_ALG_ASYNC);
	if (IS_ERR(hmac_md5)) {
		return -ENOMEM;
	}
	...
	if (err)
		goto out;
	err = crypto_hash_setkey(hmac_md5, CFG_DIGEST_SIGN_KEY, 16);
	if (err)
		goto out;

	sg_init_one(sg, in_data, len);
	err = crypto_hash_digest(&desc, sg, len, out_data);
