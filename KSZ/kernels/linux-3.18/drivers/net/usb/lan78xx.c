...
#define DRIVER_VERSION	"1.0.6.3.18.6"
...
#define MIN_SINGLE_PACKET_SIZE		(68) /* RFC 791 */
...
#define CONFIG_KSZ9897_IBA
#define CONFIG_KSZ_IBA_ONLY
#define CONFIG_KSZ_IBA

#define KSZ9897_KSZ9031_PHY 6
#define ERR_RETRY -2
...
#endif /* get_sysfs_data */

static void copy_old_skb(struct sk_buff *old, struct sk_buff *skb);

spinlock_t ksz_lock = __SPIN_LOCK_UNLOCKED(x);
static bool usb_disconnected;

#include "../ethernet/micrel/spi-ksz9897.c"

#endif /* CONFIG_LAN78XX_KSZ9897_IBA */
...
#if LINUX_VERSION_CODE <= KERNEL_VERSION(3, 18, 14)
...
#ifdef CONFIG_LAN78XX_KSZ9897_IBA
	if (dev->opened && netdev->phydev == NULL)
		return 0;
#endif
...
#ifdef CONFIG_LAN78XX_KSZ9897_IBA
static int ksz9897_ksz9031_phyid;
				      ...
				      struct ethtool_cmd *cmd)
{
	...
	if (sw_is_switch(sw)) {
		ret = mii_ethtool_gset(&dev->mii_if, cmd);
	} else {
#endif
...
#ifdef CONFIG_LAN78XX_KSZ9897_IBA
	if (sw_is_switch(sw)) {
		ret = mii_ethtool_sset(&dev->mii_if, cmd);
	} else {
...
static const struct ethtool_ops lan78xx_ethtool_ops = {
	...
	.get_settings = lan78xx_get_link_ksettings,
	.set_settings = lan78xx_set_link_ksettings,
};
...
#ifdef CONFIG_LAN78XX_KSZ9897_IBA
	if (sw_is_switch(sw)) {
		if (mii_data->phy_id == KSZ9897_KSZ9031_PHY) {
			mii_data->val_out = dev->mdiobus->read(dev->mdiobus,
				ksz9897_ksz9031_phyid , mii_data->reg_num);
			return 0;
		} else {
			phy_mii_ioctl(netdev->phydev, rq, cmd);
		}
	} else
#endif
...
static int lan8835_fixup(struct phy_device *phydev)
{
	...
	buf = phy_read_mmd_indirect(phydev, 0x8010,
				    3, phydev->addr);
	buf &= ~0x1800;
	buf |= 0x0800;
	phy_write_mmd_indirect(phydev, 0x8010, 3,
			       phydev->addr, buf);
...
static int ksz9031rnx_fixup(struct phy_device *phydev)
{
	...
	phy_write_mmd_indirect(phydev, 4, 2,
			       phydev->addr, 0x0077);
	/* RGMII RX Data Pad Skew */
	phy_write_mmd_indirect(phydev, 5, 2,
			       phydev->addr, 0x7777);
	/* RGMII RX Clock Pad Skew */
	phy_write_mmd_indirect(phydev, 8, 2,
			       phydev->addr, 0x1FF);
...
static bool fixed_phy_device_present;
static bool try_ksz9897_present;
#endif
	...
	if (!phydev || (phydev && !phydev->drv)) {
		...
		if (dev->chipid == ID_REV_CHIP_ID_7801_) {
			...
			phydev = fixed_phy_register(PHY_POLL, &fphy_status,
						    NULL);
			if (IS_ERR(phydev)) {
				netdev_err(dev->net, "fixed PHY register failed\n");
				return -EIO;
			}
			netdev_info(dev->net, "registered fixed PHY\n");
			netdev_info(dev->net, "KSZ9897 IBA Switch device\n");

			fixed_phy_device_present = true;
...
static int lan78xx_change_mtu(struct net_device *netdev, int new_mtu)
{
	...
	if (new_mtu < MIN_SINGLE_PACKET_SIZE ||
	    new_mtu > MAX_SINGLE_PACKET_SIZE)
		return -EINVAL;
...
static int lan78xx_set_mac_addr(struct net_device *netdev, void *p)
{
	...
	if (sw_is_switch(sw)) {
		struct lan78xx_net *hw_dev = dev->hw_priv;
		u8 hw_promisc = hw_dev->hw_promisc;
		u8 promisc;

		if (!memcmp(netdev->dev_addr, addr->sa_data, ETH_ALEN))
			return 0;

		memcpy(netdev->dev_addr, addr->sa_data, ETH_ALEN);

		//FIXME : Set promisc mode
		promisc = sw->net_ops->set_mac_addr(sw, netdev, hw_promisc,
				hw_dev->port.first_port);

		if (hw_dev != hw_dev->hw_priv)
			return 0;
	}
#endif /* CONFIG_LAN78XX_KSZ9897_IBA */
...
static int lan78xx_reset(struct lan78xx_net *dev)
{
	...
	if (fixed_phy_device_present || try_ksz9897_present) {
		ret = lan78xx_write_reg(dev, MAC_RGMII_ID,
					MAC_RGMII_ID_TXC_DELAY_EN_);
		ret = lan78xx_write_reg(dev, RGMII_TX_BYP_DLL, 0x3D00);
		ret = lan78xx_read_reg(dev, HW_CFG, &buf);
...
static void get_sysfs_data_(struct net_device *dev,
			    ...
			    struct ksz_port **port)
{
	struct lan78xx_net *hw_dev = netdev_priv(dev);
	struct sw_priv *hw_priv;

	hw_priv = hw_dev->parent;
	*port = &hw_dev->port;
...
{
	struct lan78xx_net *priv = netdev_priv(dev);
...
static struct lan78xx_net *sw_rx_proc(struct ksz_sw *sw, struct sk_buff *skb,
				      int *ptp_tag,
				      struct net_device **parent_dev,
				      struct sk_buff **parent_skb)
{
	struct net_device *dev;
	struct lan78xx_net *hw_dev;
	int len = skb->len;
	int rx_port = 0;
	int forward = 0;
	int tag = 0;
	...
	*parent_skb = NULL;
	...
	hw_dev = netdev_priv(dev);

	/* Internal packets handled by the switch. */
	if (!sw->net_ops->drv_rx(sw, skb, rx_port))
		return NULL;

	//FIXME : Need to check match_pkt for multi and promisc
...
static struct ksz_sw *check_avail_switch(int id)
{
...
{
	struct lan78xx_net *priv = netdev_priv(dev);
	struct mii_bus *bus = priv->phy_dev->bus;
	int err;
	int val_out = 0xffff;
...
static int prep_sw_dev(struct ksz_sw *sw, struct lan78xx_net *dev, int i,
		       int port_count, int mib_port_count, char *dev_name)
{
	...
	dev->phy_addr = sw->net_ops->setup_dev(sw, dev->net, dev_name,
					       &dev->port, i, port_count,
					       mib_port_count);
	phy_mode = dev->interface;

	if (fixed_phy_device_present) {
		phy_stop(dev->net->phydev);
		fixed_phy_unregister(dev->net->phydev);
		fixed_phy_device_present = false;
		phy_disconnect(dev->net->phydev);
		dev->net->phydev = NULL;
	}

	if (try_ksz9897_present) {
		ksz9897_ksz9031_phyid = dev->net->phydev->addr;
		phy_unregister_fixup_for_uid(PHY_KSZ9031RNX, 0xfffffff0);
		phy_unregister_fixup_for_uid(PHY_LAN8835, 0xfffffff0);
		phy_stop(dev->net->phydev);
		phy_disconnect(dev->net->phydev);
		dev->net->phydev = NULL;
		try_ksz9897_present = false;
	}

	snprintf(bus_id, MII_BUS_ID_SIZE, "sw.%d", 0);
	snprintf(phy_id, MII_BUS_ID_SIZE, PHY_ID_FMT, bus_id, dev->phy_addr);
	dev->phy_dev = phy_attach(dev->net, phy_id, phy_mode);
	if (IS_ERR(dev->phy_dev)) {
	...
	dev->mii_if.phy_id_mask = 0x1f;
	dev->mii_if.reg_num_mask = 0x1f;
	dev->mii_if.dev = dev->net;
	dev->mii_if.mdio_read = mdio_read;
	dev->mii_if.mdio_write = mdio_write;
	dev->mii_if.phy_id = dev->phy_addr;
	if ((dev->phy_dev->drv->features & PHY_GBIT_FEATURES) ==
			PHY_GBIT_FEATURES)
		dev->mii_if.supports_gmii = 1;
	return 0;
}  /* prep_sw_dev */

static int lan78xx_sw_init(struct lan78xx_net *dev)
{
	struct ksz_sw *sw = NULL;
	int port_count;
	int dev_count;
	...
	ret = prep_sw_dev(sw, dev, 0, port_count, mib_port_count, dev_label);
	if (ret < 0)
		return ret;
	/* Only the main one needs to set adjust_link for configuration. */
	dev->phy_dev->adjust_link = lan78xx_link_status_change;
	dev->phy_dev->adjust_link(dev->phy_dev->attached_dev);

	/* Point to real private structure holding hardware information. */
	dev->hw_priv = hw_priv;
	//FIXME : Need to set init promisc_reset_work

#ifdef CONFIG_KSZ_IBA_ONLY
	do {
		struct phy_device *phydev = dev->phy_dev;
 ...
 */
static int netdev_start_iba(struct ksz_sw *sw)
{
	...
	ret = lan78xx_sw_init(hw_dev);
	if (ret < 0)
		return ret;

	hw_dev->multi = false;
	hw_dev->promisc = false;
	...
	//FIXME : Set multicast and promisc modes

	sw->net_ops->open(sw);

	sw->net_ops->open_port(sw, dev, &hw_dev->port, &hw_dev->state);
	hw_dev->opened = true;

	netif_carrier_on(dev);

	phy_start(hw_dev->phy_dev);
...
static int create_sw_dev(struct net_device *dev, struct lan78xx_net *hw_dev)
{
	struct sw_priv *ks;
	struct ksz_sw *sw;
	int ret;
...
static int lan78xx_stop(struct net_device *net)
{
	...
	do {
		...
		if (sw_is_switch(sw)) {
			sw->net_ops->close(sw);
			sw->net_ops->close_port(sw, net, &dev->port);
				       ...
				       struct sk_buff *skb, gfp_t flags)
{
	...
	if (vlan_tx_tag_present(skb)) {
		...
		tx_cmd_b |= vlan_tx_tag_get(skb) & TX_CMD_B_VTAG_MASK_;
	}
...
#ifdef CONFIG_LAN78XX_KSZ9897_IBA
	struct ksz_port *port = &dev->port;
	struct ksz_sw *sw = dev->port.sw;
#endif

#ifdef CONFIG_LAN78XX_KSZ9897_IBA
	if (sw_is_switch(sw)) {
		skb = sw->net_ops->final_skb(sw, skb, net, port);
		if (!skb)
			return NETDEV_TX_OK;
	}
#endif
...
gso_skb:
	...
	switch (ret) {
	case 0:
		txq = netdev_get_tx_queue(dev->net, 0);
