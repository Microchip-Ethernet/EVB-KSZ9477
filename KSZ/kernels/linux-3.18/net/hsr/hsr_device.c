...
void hsr_check_carrier_and_operstate(struct hsr_priv *hsr)
{
...
#if 1
	/*
	 * This routine can be re-entried after the hsr_set_operstate call.
	 * And operstate can be already set to IF_OPER_UP.
	 */
	if (!again) {
		if (!last_state)
			last_state = old_operstate;

		/* Use last remembered state instead. */
		old_operstate = last_state;
	}
	++again;
#endif
	...
	--again;
	if (!again)
		last_state = master->dev->operstate;
#endif
}
...
static int hsr_add_vid(struct net_device *dev, __be16 proto, u16 vid)
{
	struct hsr_priv *hsr = netdev_priv(dev);
...
static int hsr_kill_vid(struct net_device *dev, __be16 proto, u16 vid)
{
	struct hsr_priv *hsr = netdev_priv(dev);

	if (vid > 0 && vid == hsr->vid)
		hsr->vid = 0;
	return 0;
}
...
static void send_hsr_supervision_frame(struct hsr_port *master, u8 type)
{
...
#ifdef CONFIG_HSRv1
	/* Need to pad enough to satisfy the 70 bytes with VLAN requirement. */
	tlen += 40;
#endif
	...
	if (master->hsr->vid) {
		...
		vlan->h_vlan_TCI = htons(master->hsr->vid);
...
#ifdef CONFIG_HSRv1
	hsr_stype = (typeof(hsr_stype)) skb_put(skb, sizeof(*hsr_stype));
	hsr_stype->HSR_TLV_Type = type;
	...
	if (master->hsr->redbox_addr) {
		hsr_stype = (typeof(hsr_stype))
			skb_put(skb, sizeof(*hsr_stype));
		hsr_stype->HSR_TLV_Type = HSR_TLV_REDBOX;
		hsr_stype->HSR_TLV_Length = 6;
		hsr_sp = (typeof(hsr_sp)) skb_put(skb, sizeof(*hsr_sp));
		ether_addr_copy(hsr_sp->MacAddressA, master->hsr->redbox_addr);
	}
	...
	if (skb->len < tlen) {
		memset(&skb->data[skb->len], 0, tlen - skb->len);
		skb_put(skb, tlen - skb->len);
	}
#endif
...
#if 1
	.ndo_vlan_rx_add_vid = hsr_add_vid,
	.ndo_vlan_rx_kill_vid = hsr_kill_vid,
#endif
};
...
#ifdef CONFIG_HSRv1
	hsr->sup_sequence_nr = HSR_SEQNR_START;
	ether_addr_copy(hsr->redbox_addr, hsr_dev->dev_addr);
	hsr->redbox_addr[ETH_ALEN - 1]++;
	hsr->net_id = 7;

#ifdef CONFIG_KSZ_SWITCH
	/* Support sending to both ports. */
	hsr->tx = 1;
#endif
#endif
