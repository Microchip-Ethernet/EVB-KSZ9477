...
#if 0
printk("%s %02x:%02x:%02x:%02x:%02x:%02x %04x\n", __func__,
...
#if 0
if (!is_sup) {
printk(" %d %02x:%02x:%02x:%02x:%02x:%02x ", is_sup,
ethhdr->h_source[0],
ethhdr->h_source[1],
ethhdr->h_source[2],
ethhdr->h_source[3],
...
#if 0
printk("  A  %02x:%02x:%02x:%02x:%02x\n",
hsr_sp->MacAddressA[0],
hsr_sp->MacAddressA[1],
hsr_sp->MacAddressA[2],
hsr_sp->MacAddressA[3],
hsr_sp->MacAddressA[4],
hsr_sp->MacAddressA[5]);
#endif
...
#ifdef CONFIG_HSRv1
		if (ether_addr_equal(node->MacAddressA, master->dev->dev_addr)) {
#if 0
printk("keep: %02x:%02x:%02x %lx %lx\n",
node->MacAddressA[3],
node->MacAddressA[4],
node->MacAddressA[5],
node->time_in[master->type], jiffies);
#endif
			hsr_register_frame_in(node, master, hsr->sequence_nr);
#if 0
	node->time_in[master->type] = jiffies;
	node->time_in_stale[master->type] = false;
#endif
continue;
}
#endif
...
#if 0
printk("forget: %02x:%02x:%02x %lx %lx %lx\n",
node->MacAddressA[3],
node->MacAddressA[4],
node->MacAddressA[5],
time_a, time_b, timestamp);
#endif
