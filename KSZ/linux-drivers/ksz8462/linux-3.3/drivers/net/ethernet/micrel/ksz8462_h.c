 ...
 */
static void __devinit dev_create_debugfs(struct dev_info *ks)
{
	struct dentry *root;
	char root_name[32];
...
static void __devexit dev_delete_debugfs(struct dev_info *ks)
{
...
static int __devinit netdev_init(struct net_device *netdev)
{
...
static int __devinit ks846x_probe(struct platform_device *pdev)
{
	int err = -ENOMEM;
	int irq;
...
static int __devexit ks846x_remove(struct platform_device *pdev)
{
	int i;
...
static struct platform_driver ks846x_platform_driver = {
	...
	.probe = ks846x_probe,
	.remove = __devexit_p(ks846x_remove),
