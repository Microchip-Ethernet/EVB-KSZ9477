...
#ifdef CONFIG_KSZ_SWITCH
		} else if (txq->tx_buf[index].type == FEC_TXBUF_T_SKB) {
			/* Want to keep track which device sents the frame. */
			    ...
			    struct kernel_hwtstamp_config *config)
{
	...
	if (sw_is_switch(sw) && (sw->features & PTP_HW)) {
		...
		result = ptp->ops->hwtstamp_get(ptp, config);
	}
			    ...
			    struct netlink_ext_ack *extack)
{
	...
	if (sw_is_switch(sw) && (sw->features & PTP_HW)) {
		...
		ptp = &sw->ptp_hw;
		result = ptp->ops->hwtstamp_set(ptp, config, ports);
	}
	if (result != -EOPNOTSUPP)
		return result;
#endif
