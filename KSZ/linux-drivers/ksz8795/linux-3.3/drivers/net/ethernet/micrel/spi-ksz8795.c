...
#ifndef DEVINIT
#if defined(CONFIG_SPI_FTDI) && defined(CONFIG_SPI_KSZ8795)
#define DEVINIT
#else
...
static void spi_wrreg(struct sw_priv *ks, u8 reg, void *buf, unsigned txl)
{
	...
	u16 *txw = (u16 *) hw_priv->rxd;
	int ret;

	if (!mutex_is_locked(&ks->lock))
		pr_alert("W not locked\n");
	txw[0] = ((KS_SPIOP_WR << SPI_ADDR_S) | reg) << SPI_TURNAROUND_S;
	txw[0] = cpu_to_be16(txw[0]);
...
static void spi_rdreg(struct sw_priv *ks, u8 reg, void *rxb, unsigned rxl)
{
	...
	txw[0] = ((KS_SPIOP_RD << SPI_ADDR_S) | reg) << SPI_TURNAROUND_S;
	txw[0] = cpu_to_be16(txw[0]);
...
static int DEVINIT ksz8795_probe(struct spi_device *spi)
{
	...
	for (; cnt < sw->mib_port_cnt; cnt++, pi++) {
		...
		case PORT_INTERFACE_GMII:
			if (sw->HOST_PORT == pi)
				sw->interface = PHY_INTERFACE_MODE_GMII;
			speed = 1000;
...
static int __devexit ksz8795_remove(struct spi_device *spi)
{
...
static struct spi_driver ksz8795_driver = {
	...
	.probe = ksz8795_probe,
	.remove = __devexit_p(ksz8795_remove),
};

#if defined(CONFIG_SPI_FTDI) && defined(CONFIG_ARCH_PEGASUS) && !defined(CONFIG_SPI_KSZ8795_MODULE)
static void ksz8795_late_init(void)
{
	spi_register_driver(&ksz8795_driver);
}
#endif
...
static int __init ksz8795_init(void)
{
	...
	if (spi_bus == 2)
		sprintf((char *) ksz8795_driver.driver.name, KS8795_DEV2);
#endif

#if defined(CONFIG_SPI_FTDI) && defined(CONFIG_ARCH_PEGASUS) && !defined(CONFIG_SPI_KSZ8795_MODULE)
	pegasus_register_late_call(ksz8795_late_init);
	return 0;
#else
...
#ifndef CONFIG_MICREL_KSZ8795_EMBEDDED
module_init(ksz8795_init);
...
#ifndef CONFIG_MICREL_KSZ8795_EMBEDDED
MODULE_DESCRIPTION("KSZ8795 switch driver");
