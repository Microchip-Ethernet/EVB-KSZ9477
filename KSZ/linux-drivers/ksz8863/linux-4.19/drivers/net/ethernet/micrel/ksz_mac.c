...
static int sw_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
	...
	if (result == -EOPNOTSUPP)
		result = sw_private_ioctl(dev, rq, NULL, cmd);
	return result;
}  /* sw_ioctl */
#endif
			...
			phy_interface_t phy_mode)
{
	...
	snprintf(phy_id, MII_BUS_ID_SIZE, PHY_ID_FMT, bus_id,
		 priv->phy_addr);
	phydev = phy_attach(priv->net, phy_id, phy_mode);
	if (!IS_ERR(phydev)) {
		priv->net->phydev = phydev;
		priv->mii_if.phy_id_mask = 0x1f;
		priv->mii_if.reg_num_mask = 0x1f;
		priv->mii_if.dev = priv->net;
...
static void netdev_start_iba(struct work_struct *work)
{
	...
	/* The PHY machine will try to bring the link up when it is down. */
