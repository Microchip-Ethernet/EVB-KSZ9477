 ...
 */
static void sw_set_addr(struct ksz_sw *sw, const u8 *mac_addr)
{
	uint i;
	uint n;
	struct ksz_port_info *info;
 ...
 */
static void sw_set_dev(struct ksz_sw *sw, struct net_device *dev,
		       const u8 *mac_addr)
{
	struct ksz_iba_info *iba = &sw->info->iba;
...
static void sw_forward(struct ksz_sw *sw, u8 *addr, const u8 *self, u16 proto,
	int tag)
{
	int forward = 0;
...
static int sw_drv_rx(struct ksz_sw *sw, struct sk_buff *skb, uint port)
{
	...
	if (sw->features & HSR_HW) {
		...
		if (sw->info->forward & FWD_MCAST)
			skb->pkt_type = PACKET_MULTICAST;
		ret = hsr_rcv(&sw->info->hsr, skb, port);

		/* Reset for hsr_forward_skb. */
		sw->info->forward = 0;
	...
	struct sk_buff *skb, struct ksz_sw_tx_tag *tag)
{
...
#ifdef USE_NEW_HSR
	if (info->redbox) {
		if (dev->dev == info->redbox) {

			/* Do not show transmit count in eth1. */
			skb->dev = info->dev;

			/* sw->priv_port is already set to the correct value
			 * because skb->dev points to eth1.
			 */
			return skb;
		}
	}
	...
	/* This frame is sent from HSR driver and may already be ready to send
	 * out.
	 */
	if (dev->dev == info->dev) {
		if (dev->forward && dev->forward <= 3) {
			member = dev->forward;
			goto skip_hsr;
		}
	}
	...
	if (sw->eth_cnt && (sw->eth_maps[i].proto & HSR_HW)) {
#ifdef USE_NEW_HSR
		struct hsr_port *from = info->hsr_ports[HSR_PT_MASTER];
#else
...
skip_hsr:
		...
		if (member < 3)
			member = BIT(info->ports[member - 1]);
		else
			member = info->member;
#else
		...
		member = info->member;
#endif
		set_tag_ports(sw, tag, member, false, false);
	}
	return skb;
}  /* sw_ins_hsr */
#endif
...
static int sw_open_dev(struct ksz_sw *sw, struct net_device *dev,
	struct ksz_port *port, const u8 *addr)
{
	int mode = 0;
...
static int init_sw_dev(int id, int dev_major, char *dev_name)
{
	...
	if (0 == dev_major)
		dev_major = result;
	sw_class[id] = class_create(dev_name);
	...
	uint mib_port_cnt)
{
	...
	if (sw->dev_offset) {
		...
		if (i) {
			...
			dev_addr_set(dev, sw->port_info[pi].mac_addr);
		} else {
			port_cnt = sw->mib_port_cnt;
			mib_port_cnt = sw->mib_port_cnt;
			sw->ops->acquire(sw);
...
static void ksz_remove(struct sw_priv *ks)
{
	struct ksz_sw *sw = &ks->sw;
