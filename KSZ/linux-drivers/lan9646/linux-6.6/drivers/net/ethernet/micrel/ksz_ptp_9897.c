...
static int ptp_hwtstamp_get(struct ptp_info *ptp,
			    struct kernel_hwtstamp_config *config)
{
	config->flags = 0;
	config->tx_type = ptp->tx_en ? HWTSTAMP_TX_ON : HWTSTAMP_TX_OFF;
	config->rx_filter = ptp->rx_en ? HWTSTAMP_FILTER_ALL :
			    HWTSTAMP_FILTER_NONE;
	return 0;
}  /* ptp_hwtstamp_get */

static int ptp_hwtstamp_set(struct ptp_info *ptp,
			    struct kernel_hwtstamp_config *config, u16 ports)
{
...
static int ptp_hwtstamp_ioctl(struct ptp_info *ptp, struct ifreq *ifr,
			      u16 ports)
{
	struct kernel_hwtstamp_config kconfig;
	...
	hwtstamp_config_to_kernel(&kconfig, &config);
...
static int ptp_hwtstamp_ioctl_get(struct ptp_info *ptp, struct ifreq *ifr)
{
	struct kernel_hwtstamp_config kconfig;
	struct hwtstamp_config config;

	ptp_hwtstamp_get(ptp, &kconfig);
	hwtstamp_config_from_kernel(&config, &kconfig);
			   ...
			   char *minor_name)
{
	...
	if (0 == dev_major)
		dev_major = result;
	ptp_class[id] = class_create(dev_name);
...
static void ptp_set_identity(struct ptp_info *ptp, const u8 *addr)
{
...
static void ptp_init(struct ptp_info *ptp, const u8 *mac_addr)
{
	uint i;
	uint n;
	int latency[2][3];
...
static struct ptp_ops ptp_ops = {
	...
	.hwtstamp_ioctl_get	= ptp_hwtstamp_ioctl_get,
	.hwtstamp_get		= ptp_hwtstamp_get,
	.hwtstamp_set		= ptp_hwtstamp_set,
