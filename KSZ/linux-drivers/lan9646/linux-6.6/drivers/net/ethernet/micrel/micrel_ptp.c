...
static int ptp_adjfine(struct ptp_clock_info *clock, long ppm)
{
	...
	clk_opt.drift = -scaled_ppm_to_ppb(ppm);
...
#if 0
static int ptp_adjphase(struct ptp_clock_info *clock, s32 phase)
{
#if 0
	struct ptp_clk_options clk_opt;
	int output;
...
static int ptp_getmaxphase(struct ptp_clock_info *clock)
{
}
#endif
...
static struct ptp_clock_info ptp_caps = {
	...
	.adjfine	= ptp_adjfine,
#if 0
	.adjphase	= ptp_adjphase,
	.getmaxphase	= ptp_getmaxphase,
#endif
	.adjtime	= ptp_adjtime,
