...
#ifdef KSZ_1588_PTP
		clock->servo_state = SERVO_LOCKED;
#endif
		/* Fall through. */
	case SERVO_LOCKED:
		clockadj_set_freq(clock->clkid, -ppb);
...
static int init_pmc(struct config *cfg, struct node *node, int domain_number)
{
	...
	snprintf(uds_local, sizeof(uds_local), "/var/run/phc2sys.%d",
		 getpid());
	node->pmc = pmc_create(cfg, TRANS_UDS, uds_local, 0, domain_number, transport_specific, 1);
	if (!node->pmc) {
...
int main(int argc, char *argv[])
{
	...
	while (EOF != (c = getopt(argc, argv,
				  "arc:d:s:E:P:I:S:F:R:N:O:L:M:i:u:wn:xz:l:t:T:mqvh"))) {
		switch (c) {
		case 'a':
			autocfg = 1;
			break;
