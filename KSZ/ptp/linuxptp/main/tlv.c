...
#ifdef KSZ_1588_PTP
uint8_t ieee_c37_238_id[3] = { IEEE_C37_238 };
#endif
			 ...
			 struct tlv_extra *extra)
{
	...
	case TLV_TRANSPARENT_CLOCK_PORT_DATA_SET:
	{
		...
		pds->portIdentity.portNumber =
			ntohs(pds->portIdentity.portNumber);
		pds->peerMeanPathDelay = net2host64(pds->peerMeanPathDelay);
		break;
	}
	case TLV_TIME:
	{
...
static void mgt_pre_send(struct management_tlv *m, struct tlv_extra *extra)
{
	...
	case TLV_TRANSPARENT_CLOCK_PORT_DATA_SET:
	{
		...
		pds->portIdentity.portNumber =
			htons(pds->portIdentity.portNumber);
		pds->peerMeanPathDelay = host2net64(pds->peerMeanPathDelay);
		break;
	}
	case TLV_TIME:
	{
...
static int org_post_recv(struct organization_tlv *org)
{
	...
	if (0 == memcmp(org->id, ieee_c37_238_id, sizeof(ieee_c37_238_id))) {
		...
		switch (org->subtype[2]) {
		case 1:
			d = (struct ieee_c37_238_data *)(org + 1);
			d->grandmasterID = ntohs(d->grandmasterID);
			d->grandmasterTimeInaccuracy =
				ntohl(d->grandmasterTimeInaccuracy);
			d->networkTimeInaccuracy =
				ntohl(d->networkTimeInaccuracy);
			break;
		}
	}
#endif
...
static void org_pre_send(struct organization_tlv *org)
{
	...
	if (0 == memcmp(org->id, ieee_c37_238_id, sizeof(ieee_c37_238_id))) {
		...
		case 1:
			d = (struct ieee_c37_238_data *)(org + 1);
			d->grandmasterID = htons(d->grandmasterID);
			d->grandmasterTimeInaccuracy =
				htonl(d->grandmasterTimeInaccuracy);
			d->networkTimeInaccuracy =
				htonl(d->networkTimeInaccuracy);
			break;
		}
	}
#endif
...
#ifdef KSZ_1588_PTP
static void alt_post_recv(struct alternate_time_offset_tlv *alt)
{
	alt->currentOffset = ntohl(alt->currentOffset);
	alt->jumpSeconds = ntohl(alt->jumpSeconds);
	alt->timeOfNextJump.seconds_msb =
		ntohs(alt->timeOfNextJump.seconds_msb);
	alt->timeOfNextJump.seconds_lsb =
		ntohl(alt->timeOfNextJump.seconds_lsb);
}

static void alt_pre_send(struct alternate_time_offset_tlv *alt)
{
	alt->currentOffset = htonl(alt->currentOffset);
	alt->jumpSeconds = htonl(alt->jumpSeconds);
	alt->timeOfNextJump.seconds_msb =
		htons(alt->timeOfNextJump.seconds_msb);
	alt->timeOfNextJump.seconds_lsb =
		htonl(alt->timeOfNextJump.seconds_lsb);
}
#endif
...
int tlv_post_recv(struct TLV *tlv, struct tlv_extra *extra)
{
	...
	case TLV_ALTERNATE_TIME_OFFSET_INDICATOR:
		alt_post_recv((struct alternate_time_offset_tlv *) tlv);
		break;
#endif
...
#ifdef KSZ_1588_PTP
	case TLV_ALTERNATE_TIME_OFFSET_INDICATOR:
		alt_pre_send((struct alternate_time_offset_tlv *) tlv);
		break;
#endif
	case TLV_REQUEST_UNICAST_TRANSMISSION:
	case TLV_GRANT_UNICAST_TRANSMISSION:
